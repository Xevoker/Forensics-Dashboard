<?php
    ...
    logAction($_SESSION["user_id"], "User Accessed Guymager Analysis", "guymager_analysis.php");
        ...
        if (!$evidence_id) {
            ...
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                ...
                if (empty($pythonPath) || !file_exists($pythonPath)) {
                    header("Location: guymager_analysis.php?analysis=config_error");
                    exit();
                } else {
                    $cmd = "cmd /c start /B \"\" \"$pythonPath\" \"$scriptPath\" \"$evidence_id\" >> \"$logPath\" 2>&1";
                    pclose(popen($cmd, "r"));
                    header("Location: guymager_analysis.php?analysis=started");
                    exit();
                }
            } else {
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Guymager Analysis</h1>
                    ...
                    <div class="alert alert-info" role="alert">
                        <h6 class="alert-heading"><i class="fas fa-lightbulb me-2"></i>Guymager Log Export Instructions</h6>
                        <p class="mb-2"><strong>To analyze Guymager imaging logs:</strong></p>
                        <ol class="mb-0">
                            <li>In Guymager, create a forensic image of the evidence disk/drive</li>
                            <li>After imaging, export the verification log or imaging report</li>
                            <li>Save as <strong>CSV</strong> or text format with relevant data</li>
                            <li>Upload the <code>.csv</code> file</li>
                            <li>Click "Start Analysis" to extract imaging and verification data</li>
                        </ol>
                        <hr class="my-2">
                        <small><strong>Note:</strong> Includes imaging logs, hash verification data, sector information, and integrity checks.</small>
                    </div>

                    <div class="row">
                        <div class="col-md-12">
                            <div class="card mb-4">
                                <div class="card-header">Forensic Processing Engine - Guymager</div>
                                <div class="card-body">
                                    <p class="text-muted">This page runs the Guymager evidence analyzer for case: <strong><?php echo htmlspecialchars($current_case); ?></strong></p>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body" style="max-height: 700px; overflow-y: auto;">
                            <?php
                            // Get artifacts for the case where tool = 'Guymager'
                            $stmt = $db->prepare("SELECT a.id, a.tool, a.artifact_type, a.value, a.severity, a.timestamp, a.evidence_id, e.file_name FROM artifacts a JOIN evidence e ON a.evidence_id = e.id WHERE a.case_id = ? AND a.tool = 'Guymager' ORDER BY a.timestamp DESC");
                            $stmt->execute([$current_case]);
                                    ...
                                    <table class="table table-striped table-bordered" id="artifactsTable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr data-file="<?php echo htmlspecialchars($artifact['file_name']); ?>">
                                                <td><span class="badge bg-secondary"><?php echo htmlspecialchars($artifact['tool']); ?></span></td>
            ...
            switch (data.status) {
                ...
                case 'done':
                    if (data.artifact_count === 0) {
                        return { cls: 'alert-warning', icon: 'fas fa-exclamation-circle', msg: '<strong>No artifacts extracted.</strong> Verify the file format contains imaging data.' };
                    }
